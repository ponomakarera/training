						...
						for (var j in data.response.posts[i].tags) {
							if (data.response.posts[i].tags[j].match(/(.+)→(.+)/)) {
								...
								if (!oldtag) {
									...
									if(targetElem){
										$("#"+ major).append("<li><a href='http://tagmani.tumblr.com/tagged/"+ minor +"'>"+ minor +"</a><ul id='"+ minor +"'><li><a href='http://tagmani.tumblr.com/tagged/"+ major +"→"+ minor +"'>ローカルルール</a></li></ul></li>");
