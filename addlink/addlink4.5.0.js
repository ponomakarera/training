$(function () {
    ...
    $.getJSON("http://api.tumblr.com/v2/blog/ponomakarera.tumblr.com/posts/?api_key=1Uw1n0Yvp6uylFWhR8AyhgmPTgAlvItyeOFK6XKuYcMYiygM6V&tag=%E3%83%A1%E3%83%A2&limit=20&offset="+ i * 20 +"&jsonp=?",function(data) {
		...
		for (var i in data.response.posts) {
			...
			txt = $(this).html();
			var reg = new RegExp("<dic number=\"(\d+)\" ;></dic><dic number=\"(\d+)\" ;></dic>"+word, "g");
			if (txt.match(reg)) {
				if (Number(RegExp.$1) < Number(RegExp.$2)) {
					$(this).html(txt.replace(new RegExp("<dic number=\"(\d+)\" ;></dic><dic number=\"(\d+)\" ;></dic>"+word, "g"),"<dic number='"+ RegExp.$2 +"';></dic>"+word));
				}
				else {
					$(this).html(txt.replace(new RegExp("<dic number=\"(\d+)\" ;></dic><dic number=\"(\d+)\" ;></dic>"+word, "g"),"<dic number='"+ RegExp.$1 +"';></dic>"+word));
				}
			}
			}
		}
