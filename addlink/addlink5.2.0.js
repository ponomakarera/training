$(function () {
    ...
    const detox = (ctx) => {
        ...
        Array.from(ctx.querySelectorAll([
            'img[src^="http://ponomakarera.tumblr.com/post"]',
            'area[src^="http://ponomakarera.tumblr.com/post"]'
            //'[src^="http://t.umblr.com/redirect?z="]'
        ]), link => {
            // redirector syntax;
            // http://t.umblr.com/redirect?z=<ENCODED_URL>&t=<66_DIGIT_HASH>%3D%3D
            const url = (new URL(link.href).search.match(/http:\/\/ponomakarera\.tumblr\.com\/post\/\d+\/"(.+)/) || [])[0];
