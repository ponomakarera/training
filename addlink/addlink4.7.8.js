		...
		for (var i in data.response.posts) {
			...
			console.log(txt.match(reg));
				if (Number(RegExp.$1) < Number(RegExp.$2)) {
					$(this).html(txt.replace(new RegExp("<dic number=\"(.+?)\" ;=\"\"></dic><dic number=\"(.+?)\" ;=\"\"></dic>"+word, "g"),"<dic number='"+ RegExp.$2 +"';></dic>"+word));
				}
				else {
					$(this).html(txt.replace(new RegExp("<dic number=\"(.+?)\" ;=\"\"></dic><dic number=\"(.+?)\" ;=\"\"></dic>"+word, "g"),"<dic number='"+ RegExp.$1 +"';></dic>"+word));
				}
			}
		}
		});
