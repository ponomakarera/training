$(function () {
    ...
    $.getJSON("http://api.tumblr.com/v2/blog/ponomakarera.tumblr.com/posts/?api_key=1Uw1n0Yvp6uylFWhR8AyhgmPTgAlvItyeOFK6XKuYcMYiygM6V&tag=%E3%83%A1%E3%83%A2&limit=20&offset="+ i * 20 +"&jsonp=?",function(data) {
		...
		for (var i in data.response.posts) {
		    ...
		    $(this).html(txt.replace(new RegExp("(?!<dic>.*</dic>)"+word, "g"),"<dic>"+word+"</dic>"+word));
			//txt = $(this).html();
			//$(this).html(txt.replace(new RegExp("<dic>(?!"+word+".*)</dic>"+word, "g"),"<dic>"+word+"</dic>"+word));
			}
		}
	});
		...
		for (var i in data.response.posts) {
		    ...
		    var txt = $(this).html();
			if (!($(this).hasClass(word))) {
		    $(this).html(txt.replace(new RegExp("<dic>"+word+"</dic>"+word),"<a  class='textlink' title='"+ linkbody +"'; href='"+data.response.posts[i].post_url+"' style='color:#0645ad '>"+word+"</a>"));
		    }
		}
	});
