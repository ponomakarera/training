	...
	$(window).load(function() {
		...
		$(".addlink").each(function(){
			for (var i = 0; i < array.length; i++) {
				...
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp("<span id=.wordlist.>([^\x01-\x7E]*?)<span id=.wordlist.>"+ array[i] +"</span>([^\x01-\x7E]*?)</span>", "g"),"<span id='wordlist'>$1"+ array[i] +"$2</span>"));
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp("<span id=.wordlist.>(\w*?)<span id=.wordlist.>"+ array[i] +"</span>(\w*?)</span>", "g"),"<span id='wordlist'>$1"+ array[i] +"$2</span>"));
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp("<span id=.wordlist.><span id=.wordlist.>"+ array[i] +"</span>", "g"),"<span id='wordlist'>"+ array[i]));
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp("<span id=.wordlist.>"+ array[i] +"</span></span>", "g"),array[i] +"</span>"));
			}
		});
