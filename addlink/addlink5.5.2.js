	...
	$(window).load(function() {
		...
		$(".addlink").each(function(){
			for (var i = 0; i < array.length; i++) {
				var txt = $(this).html();
				$(this).html(txt.replace(new RegExp("(?!<span id=.wordlist.>).*?"+ array[i], "g"),"<span id='wordlist'>"+array[i]+"</span>"));
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp(array[i] +".*?(?!</span>)", "g"),"<span id='wordlist'>"+ array[i] +"</span>"));
				txt = $(this).html();
				$(this).html(txt.replace(new RegExp("</span>(.*?(?!<span id=.wordlist.>).*?)"+ array[i] +"(.*?(?!</span>).*?)<span id=.wordlist.>", "g"),"$1<span id='wordlist'>"+ array[i] +"</span>$2"));
			}
		});
